...
class AdministrativeFunctionClassifier:
    ...
    def __init__(self, 
                 ...
                 output_dir: Path = Path("data/processed/phase8")):
    ...
    def load_base_data(self) -> pd.DataFrame:
        ...
        clusters = pd.read_csv(self.data_dir / "cluster_assignments_kmeans.csv")
        ...
        color_diversity.columns = ['khipu_id', 'unique_colors', 'total_color_records']
        
        base_df = base_df.merge(color_diversity, on='khipu_id', how='left')
        
        # Fill NaN values
        base_df['pendant_match_rate'] = base_df['pendant_match_rate'].fillna(0)
        base_df['has_pendant_summation'] = base_df['has_pendant_summation'].fillna(0)
        base_df['unique_colors'] = base_df['unique_colors'].fillna(0)
        base_df['total_color_records'] = base_df['total_color_records'].fillna(0)
        
        print(f"\n✓ Merged dataset: {len(base_df)} khipus with {len(base_df.columns)} features")
        return base_df
    
    def extract_structural_features(self, base_df: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        structural_features['has_summation'] = (base_df['pendant_match_rate'] > 0.5).astype(int)
        
        # Numeric behavior
        structural_features['numeric_coverage'] = base_df['has_numeric'].fillna(0)
        structural_features['avg_numeric_value'] = base_df['avg_numeric_value'].fillna(0)
        
        # Structural complexity
        structural_features['node_density'] = base_df['density']
        structural_features['leaf_ratio'] = base_df['num_leaves'] / (base_df['num_nodes'] + 1)
        
        # Aggregation layers (depth > 2 indicates multi-level aggregation)
        structural_features['has_aggregation'] = (base_df['depth'] >= 3).astype(int)
        
        # Structural entropy (diversity of branching)
        # Use coefficient of variation of branching as proxy
        structural_features['structural_complexity'] = base_df['max_degree'] / (base_df['avg_branching'] + 1)
    ...
    def extract_chromatic_features(self, base_df: pd.DataFrame, 
        ...
        for idx, khipu_id in enumerate(base_df['khipu_id']):
            ...
            chromatic_features.loc[idx, 'color_cord_ratio'] = len(khipu_colors) / max(cord_count, 1)
            
            # Color diversity
            chromatic_features.loc[idx, 'unique_color_count'] = khipu_colors['color_cd_1'].nunique()
            
            # Multi-color complexity (cords with multiple colors)
            multi_color_count = (khipu_colors['color_cd_2'].notna()).sum()
            chromatic_features.loc[idx, 'multi_color_ratio'] = multi_color_count / max(len(khipu_colors), 1)
            ...
            if len(khipu_hierarchy) > 0:
                ...
                # Primary cord colors
                primary_colors = merged[merged['CORD_LEVEL'] == 1]['color_cd_1'].nunique()
                chromatic_features.loc[idx, 'primary_color_diversity'] = primary_colors
                
                # Pendant colors (level 2)
                pendant_colors = merged[merged['CORD_LEVEL'] == 2]['color_cd_1'].nunique()
                chromatic_features.loc[idx, 'pendant_color_diversity'] = pendant_colors
                
                # Color transitions (changes between parent-child)
                chromatic_features.loc[idx, 'color_transitions'] = self._count_color_transitions(merged)
    ...
    def _count_color_transitions(self, hierarchy_with_colors: pd.DataFrame) -> int:
        ...
        for _, row in hierarchy_with_colors.iterrows():
            if pd.notna(row['ATTACHED_TO']):
                ...
                if len(parent) > 0 and pd.notna(row['color_cd_1']) and pd.notna(parent.iloc[0]['color_cd_1']):
    ...
    def perform_structural_clustering(self, structural_features: pd.DataFrame,
        ...
        feature_cols = [col for col in structural_features.columns if col != 'khipu_id']
        ...
        # Analyze clusters
        cluster_stats = self._analyze_clusters(structural_features, cluster_labels, feature_cols)
    ...
    def _analyze_clusters(self, features_df: pd.DataFrame, 
        ...
        for _, row in stats_df.iterrows():
            print(f"  Cluster {int(row['cluster_id'])}: {int(row['count'])} khipus ({row['pct']:.1f}%)")
    ...
    def build_integrated_classifier(self, 
        ...
        try:
            function_data = pd.read_csv(self.data_dir / "khipu_function_classification.csv")
            ...
            print("  Warning: No ground truth labels found, using cluster-based heuristic")
        ...
        if 'predicted_function' not in integrated.columns or integrated['predicted_function'].isna().all():
            print("  Warning: Creating synthetic labels from cluster assignments")
            integrated['predicted_function'] = integrated['cluster'].astype(str)
        
        # Fill missing values
        integrated = integrated.fillna(0)
        
        # Define feature sets
        structural_cols = [col for col in structural_features.columns if col != 'khipu_id']
        chromatic_cols = [col for col in chromatic_features.columns if col != 'khipu_id']
    ...
    def generate_final_typology(self, 
        ...
        typology['predicted_function'] = self.integrated_features['predicted_function'].astype(str)
...
def run_phase8_analysis():
    ...
    chromatic_features = classifier.extract_chromatic_features(base_df, color_data)
    
    # Perform structural clustering
    structural_clusters = classifier.perform_structural_clustering(structural_features)
    ...
    # Save all results
    classifier.save_results(structural_clusters, classifier_results, final_typology)
