...
if __name__ == "__main__":
    ...
    print(f"\nVariance Statistics:")
    print(f"  Mean flexibility score: {variance['flexibility_score'].mean():.3f}")
    print(f"  Mean constraint score: {variance['constraint_score'].mean():.3f}")
    
    print(f"\nDimensional Variance:")
    print(f"  Numeric CV (mean): {numeric['cv'].mean():.2f}")
    print(f"  Color within-khipu evenness: {color['standardization_score'].mean():.2f}")
    ...
    top_flexible = variance.nlargest(5, 'flexibility_score')[['khipu_id', 'flexibility_score', 'design_class']]
    print(top_flexible.to_string(index=False))
    
    print(f"\nTop 5 Most Constrained Khipus:")
    top_constrained = variance.nlargest(5, 'constraint_score')[['khipu_id', 'constraint_score', 'design_class']]
